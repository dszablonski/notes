...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\draw (2,0.5,0) -- (2,0,0);
		\node at (1,0.5,1.5) {$A$};
		\draw[<->] (2.5,0.5,1.5) -- (2.5,0,1.5) node[midway, anchor=west] {$\lambda$};
	\end{tikzpicture}
...
\end{equation}
Some notes on viscosity,
\begin{itemize}
	\item We can rewrite viscosity as
	\begin{equation}
		\eta = \frac{1}{3\sqrt{2}}\frac{\left<v\right>m}{\sigma}
	\end{equation}
	by eq. \eqref{eq:mean free path}.
	\item Thus, $\eta$ is independent of pressure, which holds under the conditions $\sqrt{\sigma} << \lambda << L$.
	\item $\eta \propto \left<v\right> \propto \sqrt{T}$.
\end{itemize}
\section{Transport of Energy (Thermal Conductivity)}
\begin{figure}[b]
	\centering
	\begin{tikzpicture}[scale=1.25]
		...
		\draw (2,0.5,3) -- (2,0,3);
		\draw (2,0.5,0) -- (2,0,0);
		\draw[<->] (2.5,0.5,0) -- (2.5,0,0) node[midway, anchor=west] {$\lambda$};
		...
		\draw (2,0.5,0) -- (2,1,0);
		\node at (1,1,1.5) {$A$};
		\draw[<->] (2.5,0.5,0) -- (2.5,1,0) node[midway, anchor=west] {$\lambda$};
		\node at (-0.5,0.5,3) {$z_0$};
		\node at (-0.65,1,3) {$z_0 + \lambda$};
		\node at (-0.65,0,3) {$z_0 - \lambda$};
	\end{tikzpicture}
	\caption{Thin slice of gas of volume $2\lambda A$ centered on the $z_0$ plane.}
	\label{fig:conductivity}
\end{figure}
We will assume position dependent temperature, i.e., $T \equiv T(\vb{r})$. Heat flows along a temperature gradient, thus we can define the heat flux as,
\begin{equation}
	\vb{q} = -\kappa \grad \vb{T}.
\end{equation}
where $\kappa$ is the conductivity. If we approximate our heat flow as begin 1 dimensional, we can write,
\begin{equation}
	q = -\kappa \pdv{T}{z}.
\end{equation}
