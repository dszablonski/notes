...
\newcommand{\dasheddoublearrowlabel}[5]{
	% accepts a start point, end point, spacing, gradient and label
	\path let \p1=#1 in coordinate (start) at ({\x1 + #3* cos(#4)},{\y1-#3 * sin(#4)});
	\path let \p2=#2 in coordinate (end) at ({\x2 + #3* cos(#4)},{\y2-#3 * sin(#4)});
	
	\draw[latex-latex, dashed]  (start) -- (end) node[midway,fill=white]{#5};
}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\draw[thick] plot[domain=0:2*pi, samples=100]  (\x/pi,{sin(5 * \x r)}); 
		\draw[very thick] (2, 2) -- (2, -2);
		\draw[xshift=2cm] plot[domain=0:2*pi, samples=100] (\x/pi,{pow(e, -0.2*\x) * sin(5*\x r)});
		\draw[xshift=2cm] plot[domain=0:2*pi, samples = 100] (\x/pi, {pow(e,-0.2*\x)});
		\node at (3, 1) {$e^{-\frac{z}{\delta}}$};
		\draw[xshift=4cm, thick] plot[domain=0:1*pi, samples = 100] (\x/pi, {pow(e,-0.2*2*pi)*sin(5*\x r)});
		\draw[very thick] (4,2) -- (4, -2);
		\node at (3, -2) {\small Conductor};
		\node at (0.8, 1.5) {\small Vacuum};
		\node at (5, 1.5) {\small Vacuum};
	\end{tikzpicture}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\draw (0,0) rectangle (7.1,0.5);
		\draw (0,4) rectangle (7.1, 4.5);
		\doublearrowlabel{(0,0.6)}{(0,3.9)}{0}{0}{$a$};
		\begin{scope}[decoration={
				markings,
				mark=at position 0.5 with {\arrow{>}}}
			] 
		\draw[postaction={decorate}] (0.1,0.5) -- (2.1,4) node[midway, anchor=east] {$\vb{E}_{\text{out}}$};
		\draw[postaction={decorate}] (2.1,4) -- (4.1,0.5) node[midway, anchor=east] {$\vb{E}_{\text{in}}$};
		\draw[postaction={decorate}] (4.1,0.5) -- (6.1,4);
		\draw[->] (6.1,4) -- (7.1, 2);
		\end{scope}
		\draw[->] (1.1,2.25) --(1.6,1.9) node[anchor=west] {$\vb{k}, c$};
		\draw[->] (3.1,2.25) -- (3.6,2.55) node[anchor=north west] {$\vb{k}, c$};
		\node[anchor=north west] at (2.2,4) {$\theta$};
		\centerarc[black](2.1,4)(360:299:0.6);
		\doublearrowlabel{(0.3,0.8)}{(3.9,0.8)}{0}{0}{$\lambda_g$};
		%\draw[dashed] (3.1,2.25) -- (6.1,4) node[midway, above] {$\lambda_0$};
		\dasheddoublearrowlabel{(3.1,2.25)}{(6.1,4)}{0}{0}{$\lambda_0$};
		\draw[-Latex] (7, 2.25) -- (7.5,2.25) node[anchor=west] {$\vu{z}$};
	\end{tikzpicture}
	\caption{A rectangular waveguide consisting of a long rectangular tube of radius $a$.}
	\label{fig:waveguide1}
\end{figure}
Consider figure \ref{fig:waveguide1}. Electromagnetic waves of wavelength $\lambda_0$ reflect between the conductor at angles $\theta$. The phase velocity of the wave in the waveguide is given by,
\begin{equation}
	v_p = \frac{c}{\sin\theta}.
\end{equation}
The distance between reflections $\lambda_g$ is given by,
\begin{equation}
	\lambda_g = \frac{\lambda_0}{\sin\theta}.
\end{equation}
We can relate the speed of light and phase velocity to the two wavelength of light and the waveguide wavelength,
\begin{align}
	c = \lambda_0 f && v_p = \lambda_g f.
\end{align}
By trigonometric considerations, we can write,
\begin{equation}
	\cos\theta = \frac{\lambda_0}{2a}
\end{equation}
from which we find the \textit{cutoff wavelength}, such that the wavelength of light passing through the waveguide must satisfy,
\begin{equation}
	\lambda_0 \leq 2a
\end{equation}
in order to propagate through the waveguide. 
\begin{figure}
	\centering
	\begin{tikzpicture}
		\draw (0,0,0) -- (4,0,0) -- (4,3,0) -- (0,3,0) -- (0,0,0);
		\draw (0,3,0) -- (0,3,-4);
		\draw (4,3,0) -- (4,3, -4);
		\draw (4,0,0) -- (4,0,-4);
		\draw (0.5,0.5,0) -- (3.5,0.5,0) -- (3.5,2.5,0) -- (0.5,2.5,0) -- (0.5,0.5,0);
		\doublearrowlabel{(0.5,0.25,0)}{(3.5,0.25,0)}{0}{0}{$a$};
		\doublearrowlabel{(0.3,0.5,0)}{(0.3,2.5,0)}{0}{0}{$b$};
		\draw[->] (0,0,0.2) -- (0,0,2) node[anchor=north east] {$\vu{z}$};
		\draw[->] (4.2,0,0) -- (6,0,0) node[anchor=west] {$\vu{x}$};
		\draw[->] (0,3.2,0) -- (0,5.2,0) node[anchor=south] {$\vu{y}$};
	\end{tikzpicture}
	\caption{Rectangular waveguide of dimensions $a$ and $b$.}
\end{figure}
